import random
from NEAT.NEAT_implementation.Genotype.connection import Connection
from NEAT.NEAT_implementation.Genotype.node import Node
...
class Genome:
    ...
    def addNodeMutation(self):
        newNode = self.addNode('hidden')
        existingConnection = self.getRandomConnection()

        self.addConnection(existingConnection.inputNodeId, newNode.id, weight=1)
        self.addConnection(newNode.id, existingConnection.outputNodeId, weight=existingConnection.weight)
