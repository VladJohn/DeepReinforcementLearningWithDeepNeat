...
import numpy as np
import torch
import NEAT.NEAT_implementation.Utils.utils as utils
from NEAT.NEAT_implementation.Genotype.genome import Genome
from NEAT.NEAT_implementation.Species.species import Species
from NEAT.NEAT_implementation.Crossover.crossover import crossover
from NEAT.NEAT_implementation.Mutation.mutation import mutate
...
class Population:
    ...
    def setInitialPopulation(self):
        ...
        for i in range(self.configuration.POPULATION_SIZE):
            ...
            for j in range(self.configuration.NUM_INPUTS):
                node = freshGenome.addNode('input')
                inputs.append(node)

            for j in range(self.configuration.NUM_OUTPUTS):
                node = freshGenome.addNode('output')
                outputs.append(node)
