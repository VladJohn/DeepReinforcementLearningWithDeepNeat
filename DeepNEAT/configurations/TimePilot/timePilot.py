...
class TimePilotConfig:
    ...
    def fitness(self, genome):
        ...
        trainingData = np.load('timePilot.npy', allow_pickle=True)
        ...
        for epoch in tqdm(range(self.EPOCHS)):
            for i in tqdm(range(0, len(trainX), self.BATCH_SIZE)):
                batchX = trainX[i:i+self.BATCH_SIZE].view(-1, 128)
        ...
        with torch.no_grad():
            for i in range(len(testX)):
                ...
                net_out = phenotype(testX[i].view(-1, 128))[0]
    ...
    def build_test_data(self):
        ...
        np.save('timePilot.npy', trainingDataSave)
